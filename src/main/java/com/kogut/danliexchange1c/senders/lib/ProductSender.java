...
@Component
public class ProductSender extends AbstractSender<ProductDTO> implements ISender<ProductDTO> {

    @Autowired
    public ProductSender(ISettingsUtil settingsUtil, RestTemplate restTemplate) {
        super(settingsUtil, restTemplate);
    }

    @Override
    public HttpStatus send(ProductDTO productDTO) {
        return super.send(productDTO, "pathProduct");
    }
