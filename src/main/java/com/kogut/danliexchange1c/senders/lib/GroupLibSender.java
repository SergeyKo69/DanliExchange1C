...
@Component
public class GroupLibSender implements ISender<GroupLibDTO> {
    ...
    public HttpStatus send(GroupLibDTO groupLibDTO) {
        HashMap<String, String> settings = settingsUtil.getSettings(ClientDBEnum.valueOf(groupLibDTO.getClientDB()));
        StringBuilder url = new StringBuilder();
        url.append(settings.get("address")).append("/").append(settings.get("pathGroupLib"));
        HttpEntity<GroupLibDTO> request = new HttpEntity<>(groupLibDTO, settingsUtil.createHeaders(settings.get("userName"), settings.get("password")));
        try {
